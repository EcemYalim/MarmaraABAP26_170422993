CLASS zcl_170422993_rest_client_02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      run_demo IMPORTING io_out TYPE REF TO if_oo_adt_classrun_out,
...
CLASS zcl_170422993_rest_client_02 IMPLEMENTATION.
...
*    io_out->write( user_delete(  ) ).
  ...
  METHOD user_get.
    rv_text = execute_request(
    iv_method = 'GET'
    iv_url = gc_base_url && '/user/demo_170422993'
    ).
  ENDMETHOD.
...
***************************************************
        ...
        rv_text = |HTTP { ls_status-code } { ls_status-reason }| && lo_response->get_text(  ).
